       ...
       int entryNotFound, int accessDeniedByUser),
      {
          ...
          const reportSuccess = (val) => {
              ...
              if (isWrite) {
                  ...
                  console.log("QtKeychain: Read complete");
                  const res = toCStr(val);
          ...
          overlay.querySelector("[id='qtk-cancel']").onclick = () => {
              ...
              reportError(accessDeniedByUser, "User cancelled");
          };
              ...
              if (isWrite) {
                  if (useNativeStore) {
                      navigator.credentials
                          ...
                          .catch(err => console.error("QtKeychain:", err))
                          .finally(() => reportSuccess());
                  } else {
                      setTimeout(() => reportSuccess(), 100);
                  }
              } else {
                  if (passInput.value) {
                      reportSuccess(passInput.value);
                  } else {
                      reportError(entryNotFound, "Password cannot be empty");
                  }
              }
          };
